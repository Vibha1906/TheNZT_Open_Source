...
const HighlightedCode = ({ language, children }: CodeSnippetProps) => {
  ...
  return (
    <pre className="m-0">
      <code ref={codeRef} className={`language-${language} font-mono text-sm rounded-b-md block`}>
        {children}
      </code>
    </pre>
  );
...
export default function CodeSnippet({ ...props }: CodeProps) {
  ...
  const nonHighlightedCode = showSyntaxHighlighter ? null : (
    <div className={cn('rounded-b-md overflow-x-auto bg-accent', code && 'p-2')}>
  ...
  return (
    <Card className="relative">
      <CardHeader className="flex flex-row items-center justify-between py-1 px-4">
        <span className="text-sm text-muted-foreground">{match?.[1] || 'Raw code'}</span>
